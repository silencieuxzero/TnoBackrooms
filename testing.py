...
class StartPage(BasePage):
    def create_widgets(self):
        ...
        # 设置内容
        tk.Label(self, text="欢迎回来", font=("微软雅黑", 15)).grid(row=1, column=0, columnspan=4, pady=30)
        ...
        # 配置最后一行以扩展到窗口底部
        self.grid_rowconfigure(2, weight=1)  # 第三行扩展
        self.grid_rowconfigure(3, weight=0)  # 按钮所在行不扩展

        # 计算每行应该有多少个按钮
        buttons_per_row = 4  # 每行放置4个按钮
        ...
        for i, item in enumerate(options):
            ...
            tk.Button(self, text=item[2],
                      ...
                      font=("微软雅黑", 10),  # 指定字体
                      command=command
                      ).grid(row=row, column=column, padx=10, pady=10, sticky="s")
