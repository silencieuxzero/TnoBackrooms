...
class PoliticalPanel(BasePage):
    ...
    def setup_gui(self):
        ...
        # 设置项
        politicaloptions = [
            ("增加支持率", self.increase_support_rate_p, "增加MPP-P支持率"),
            ("增加支持率", self.increase_support_rate_c, "增加MPP-C支持率"),
            ("增加支持率", self.increase_support_rate_u, "增加MPP-U支持率"),
            ("减少支持率", self.decrease_support_rate_p, "减少MPP-P支持率"),
            ("减少支持率", self.decrease_support_rate_c, "减少MPP-C支持率"),
            ("减少支持率", self.decrease_support_rate_u, "减少MPP-U支持率"),
        ]
    ...
    # 以下为减支持率
    def decrease_support_rate_p(self):
        """ 减少支持率的方法 """
        current_rate_p = self.support_rate_p.get()
        new_rate = max(current_rate_p - 1, 0)  # 每次减少1%，最小为0%
        self.support_rate_p.set(new_rate)

    def decrease_support_rate_c(self):
        """ 减少支持率的方法 """
        current_rate_c = self.support_rate_c.get()
        new_rate = max(current_rate_c - 1, 0)
        self.support_rate_c.set(new_rate)

    def decrease_support_rate_u(self):
        """ 减少支持率的方法 """
        current_rate_u = self.support_rate_u.get()
        new_rate = max(current_rate_u - 1, 0)
        self.support_rate_u.set(new_rate)
